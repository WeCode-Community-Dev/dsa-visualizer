...
export const generateSelectionSortSteps = (array) => {
  ...
  for (let i = 0; i < n; i++) {
    ...
    for (let j = i + 1; j < n; j++) {
      steps.push({ type: 'compare', indices: [j, minIdx], values: [arr[j], arr[minIdx]], minVal: arr[minIdx] });
      if (arr[j] < arr[minIdx]) {
        steps.push({ type: 'revert', indices: [minIdx] }); // Unmark old min
        minIdx = j;
        steps.push({ type: 'highlight-min', indices: [minIdx], val: arr[minIdx] }); // Mark new min
      } else {
...
export const generateInsertionSortSteps = (array) => {
  ...
  for (let i = 1; i < n; i++) {
    let key = arr[i];
    let j = i - 1;
    steps.push({ type: 'select', indices: [i], val: key }); // Current element to insert

    while (j >= 0 && arr[j] > key) {
      steps.push({ type: 'compare', indices: [j, j + 1], values: [arr[j], key] });
...
function partition(arr, low, high, steps) {
  let pivot = arr[high];
  steps.push({ type: 'highlight-min', indices: [high], val: pivot, message: `Pivot chosen: ${pivot}` }); // Highlight pivot
  let i = (low - 1);

  for (let j = low; j <= high - 1; j++) {
    steps.push({ type: 'compare', indices: [j, high], values: [arr[j], pivot], pivotVal: pivot });
    if (arr[j] < pivot) {
      i++;
